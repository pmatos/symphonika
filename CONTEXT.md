...
**Eligible Issue**:
An open issue that matches a Dispatch Project's required labels, avoids excluded labels, is not already claimed by the orchestrator, is not suppressed by the newest Run's durable `no_workspace_changes` verdict, and has no unresolved GitHub-native issue dependency (see Dependency Gate below).
