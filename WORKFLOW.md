...
- If you discover that the issue is a duplicate of already-merged work, or is already fixed, follow step 3 (comment + close). If it is blocked or ambiguous for any other reason, follow step 9 (comment + `EVIDENCE.md` + exit) instead — these are different outcomes and use different `gh` commands.
