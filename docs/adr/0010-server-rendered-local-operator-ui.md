...
Amended by ADR-0055: the "separate frontend application" this ADR rules out means a standalone single-page app with its own build and routing, not client-side enhancement within a server-rendered page. A server-rendered operator page may embed a self-contained, read-only interactive visualization (e.g. the workflow-graph view) that degrades gracefully when its scripts are unavailable.
