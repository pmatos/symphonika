...
The Project credential inventory is explicit: the effective tracker token for the Project and,
...
Issue Runs and Routine Firings apply those resolved values before persisting provider-authored
files, provider-event rows, or provider-derived terminal evidence. Symphonika does not classify
every inherited environment value as a credential: provider-native credentials and unrelated
operator variables have no orchestrator-owned configuration boundary from which to resolve a
reliable inventory.
