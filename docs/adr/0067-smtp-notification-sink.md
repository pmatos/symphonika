...
the message can include the canonical Routine Outcome in one-line-per-project style, final state,
terminal reason, duration, branch, and discovered PR numbers. This slice does not notify on issue
Runs, Routine Skips, or daemon health.
