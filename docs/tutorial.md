...
- a `symphonika.yml` service config pointed at one of your repositories
- a `WORKFLOW.md` workflow contract that the agent will execute
...
You should see the top-level command list (`doctor`, `init-project`, `daemon`,
`smoke`, `status`, `poll-now`, `runs`, `show-run`, `cancel`, `clear-stale`,
...
## 3. Set up a working directory for your deployment

Symphonika reads its service config from the current directory. Pick a directory
that will hold your config and the local state directory; this does **not** need
to be inside the Symphonika source tree, and for most setups it should not be.

```sh
mkdir -p ~/symphonika-deploy
cd ~/symphonika-deploy
```

Add `.symphonika/` to that directory's `.gitignore` (or to your home-directory
ignore file). The state root holds the SQLite database, raw provider logs, and
issue worktrees — it is local-only and can be regenerated.

---

## 4. Write your `symphonika.yml`

Create a `symphonika.yml` in the working directory. The minimum viable shape
for one Project against one GitHub repo looks like this — replace the
`owner`, `repo`, and `remote` values with yours:
...
projects:
  - name: my-app
    ...
    priority:
      ...
      default: 99
    workspace:
      root: ./.symphonika/workspaces/my-app
      git:
...
## 6. Write your `WORKFLOW.md`

The workflow contract is the prompt the agent receives, with placeholders that
Symphonika fills in at dispatch time. Available placeholders are listed in
[SPEC.md §5.3](../SPEC.md#53-templating); the most useful are `{{issue.number}}`,
`{{issue.title}}`, `{{issue.body}}`, `{{branch.name}}`, and `{{workspace.path}}`.
...
```sh
symphonika workflow validate --config symphonika.yml
```
...
- *"workflow file not found"* — the `workflow:` path is relative to the directory
  containing `symphonika.yml`. Check the working directory.
...
```sh
symphonika init-project --config symphonika.yml --yes
```
...
```sh
symphonika smoke --config symphonika.yml
```
...
4. A workspace being prepared under
   `.symphonika/workspaces/my-app/issues/<n>-<slug>/`.
...
```sh
symphonika runs --config symphonika.yml --limit 20
symphonika show-run <run-id> --config symphonika.yml
```
...
(`.symphonika/workspaces/my-app/issues/<n>-<slug>/`); the run branch is named
...
```sh
symphonika daemon --config symphonika.yml
```
...
```sh
symphonika status --config symphonika.yml --watch
```

Force an immediate reconcile-and-poll without waiting for the next interval:

```sh
symphonika poll-now --config symphonika.yml
```

Raise log verbosity (useful while iterating on the workflow):

```sh
PINO_LOG_LEVEL=debug symphonika daemon --config symphonika.yml
```

Cancel a stuck run by id:

```sh
symphonika cancel <run-id> --config symphonika.yml
```

Clear a stale claim left behind from a previous attempt:

```sh
symphonika clear-stale my-app 42 --config symphonika.yml --yes
```
...
`symphonika.yml` and `WORKFLOW.md` are both reloadable. The daemon re-reads
them on every tick and on every `poll-now`. A valid edit applies to future
attempts; in-flight runs keep the prompt and content hash captured when the
attempt started. An invalid edit is surfaced in logs and operator status, and
the daemon keeps using the last known good snapshot.
