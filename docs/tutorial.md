...
(Codex, Claude, or Oh My Pi) against it, and records the result. By the end you will have:
...
- **`agent.provider`** picks Codex, Claude, or OMP per Project. The `providers:` block
...
- *"provider command not on PATH"* — the selected `codex`, `claude`, or `omp` binary cannot be
  resolved. Check it with `command -v <name>`. For a Bun-installed OMP, confirm `~/.bun/bin` is in
...
## 14. Switching providers, or running several

`agent.provider` is per-Project, and an agent action in a raw FSM Workflow may override it. The
simplest way to compare OMP, Claude, and Codex on separate workloads is to copy your Project entry,
change `name`, `workspace.root`, and `agent.provider`, and give each copy a different eligibility
label, such as `agent-ready-omp`. All Projects dispatch from the same daemon.

If you flip an *existing* Project to another provider, finish any in-flight
