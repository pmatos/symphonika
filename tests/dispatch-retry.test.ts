...
describe("dispatch retry policy", () => {
  ...
  it("cancels a scheduled retry when a dependency becomes an open blocker during backoff", async () => {
    ...
    const githubIssuesApi = {
      ...
      // The initial poll's own dependency fetch shares this mock, so the
      // issue must dispatch unblocked the first time around; only the
      // retry's refreshIssue (later calls) sees the blocker having been
      // linked on GitHub during the backoff.
    ...
    const daemon = await startDaemon({
      agentProviders: { codex: provider },
      createRunId: () => "run-retry-dependency-eligibility",
      cwd: root,
      env: { GITHUB_TOKEN: "secret-token" },
      githubIssuesApi,
      lifecyclePolicy: fastRetryPolicy,
    ...
    try {
      ...
      try {
        const stored = database
          .prepare("select state, cancel_reason from runs where id = ?")
          .get("run-retry-dependency-eligibility") as {
          state: string;
          cancel_reason: string;
        };
        expect(stored.state).toBe("cancelled");
