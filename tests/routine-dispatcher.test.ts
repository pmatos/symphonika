...
describe("RoutineFiringDispatcher", () => {
  ...
  it("settles a fan-out target whose provider is not registered so a surviving sibling can still notify", async () => {
    const root = await makeTempRoot();
    const stateRoot = path.join(root, ".symphonika");
    const runStore = openRunStore({ stateRoot });
    const notifications: RoutineFanoutNotification[] = [];
    ...
    try {
      const result = await dispatchDueRoutines({
        activeRuns: new ActiveRunRegistry(),
        // "omp" is intentionally absent from agentProviders: beta's target
        // simulates a Project that defaults to an unconfigured provider.
        agentProviders: { codex: provider },
        configDir: root,
      ...
      // The whole point of this test: a stuck 'pending' target on beta must
      // not block alpha's succeeded leg from ever notifying.
      expect(notifyRoutineFanout).toHaveBeenCalledTimes(1);
      expect(
        notifications[0]?.fanout.targets.map((target) => ({
          disposition: target.disposition,
        ...
        },
        {
          disposition: "skipped",
          projectName: "beta",
          skipReason: "provider_not_registered: omp"
        }
      ]);
      // Unlike overlap/concurrency_cap skips, the routine itself must stay
      // untouched (still active, still due) so it retries once the
      // operator registers the provider — see dispatcher.ts's comment on
      // skipRoutineFanoutTarget.
      expect(runStore.listRoutines({ project: "beta" })[0]).toMatchObject({
        lastSkipReason: null,
        state: "active"
      });
